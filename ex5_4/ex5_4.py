...
names_list = np.array([str(i) for i in range(1, n_cities + 1)])
...
def run_single_experiment(experiment, use_ma):
    ...
    for generation in range(n_generations):
        ...
        if use_ma:
            mutated_pop = [
                two_opt(individual, cities_dict, n_cities) for individual in mutated_pop
            ]
...
def run_ex_5_4_parallel(use_ma, max_workers=10):
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
        futures = [
            executor.submit(run_single_experiment, experiment, use_ma)
...
if __name__ == "__main__":
    ...
    print(
        f"Mean Best Fitness for EA: {mean_ea}, Best Fitness: {best_ea}, time taken: {EA_time}"
    )
    print(
