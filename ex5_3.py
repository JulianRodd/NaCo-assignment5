...
def string_search_GA(T, u, length, target_string):
    ...
    for x in range(T):
        ...
        population = [
            "".join(
                [alphabet[random.randint(0, len(alphabet) - 1)] for j in range(length)]
            )
            for i in range(N)
        ]
        generation = 0
        ...
        while target_string not in population:
            ...
            for parent in population:
                fitnesses[parent] = (
                    sum(
                        [
                            1 if target_string[i] == char else 0
            ...
            print(
                f"Highest fitness in generation {generation} is {max(fitnesses.values())}"
            )
            ...
            for i in range(int(N / K / K)):
                ...
                for i in range(2):
                    ...
                    if random.random() <= pc:
                        ...
                        children = [
                            p1[: crossover_index + 1] + p2[crossover_index + 1 :],
                            p2[: crossover_index + 1] + p1[crossover_index + 1 :],
                        ]
    ...
    return (
        (sum(target_times) / len(target_times)) if target_times else None
    ), target_times
...
def main():
    ...
    for u in u_list:
        avg_num_generations, target_times = string_search_GA(
            100, u, length, target_string=target_string
        )
        print(
            f"It took on average {avg_num_generations} generations to find the target string."
        )

        data = {"target_generation": target_times}
